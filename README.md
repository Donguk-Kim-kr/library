...
코드를 Push한 직후 문제를 발견하면 수정이 쉽지만, 배포 후에 발견하면 롤백,
긴급 대응, 고객 영향 등 비용이 기하급수적으로 커진다.
...
- 보통 읽을 때 `야믈` 또는 `야말` 이라고 읽는다.
- YAML은 프로그래밍 언어가 아니라 `정리된 메모`에 가깝다.
...
- `runs-on`이 `test`보다 더 안쪽. (`runs-on`은 `test`에 속한 세부 설정)
- `탭(Tab)` 키를 누르면 에러가 나는 경우가 많다.(스페이스바 권장)
...
on:
  ...
  - pull_request
```
- `on`이라는 키의 값이 `push`, `pull_request`여러 개라는 뜻(배열)
...
jobs:                               # 무엇을 실행할 것인가?
  job-name:                         # 작업 이름
    ...
    steps:                          # 실행 단계들 (순차 실행)
      - uses: sctions/checkout@v4   # 코드 체크 아웃
